...
# Install Ansible EDA (Event Driven Ansible)
echo "--- Installing Ansible EDA dependencies ---"
if [[ $PM == "apt" ]]; then
    sudo apt install -y openjdk-17-jdk python3-pip
else
    sudo $PM install -y java-17-openjdk python3-pip
fi

echo "--- Installing ansible-rulebook ---"
pip3 install ansible-rulebook --break-system-packages

echo "--- Installing ansible.eda collection ---"
